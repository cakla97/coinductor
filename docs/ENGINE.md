...
Detailed behaviour of the headless `trading_agent` engine: modes, strategy
decisions, previews, bankroll, research, and the guarded execution paths.

Split out of the README so the front page can stay about installing and
running Coinductor. Nothing here changed in the move.

For day-to-day operation see [RUNBOOK.md](RUNBOOK.md); for keys, IP allowlists
and provider presets see [OPERATIONS.md](OPERATIONS.md).
...
For day-to-day operation, use [docs/RUNBOOK.md](RUNBOOK.md). It lists the safe monitoring
command, guarded submit commands, and the report sections to check before taking action.
The desktop and open-source direction is tracked in [docs/ROADMAP.md](ROADMAP.md),
...
[docs/AI_PROVIDER_HANDOFF.md](AI_PROVIDER_HANDOFF.md).
...
[docs/COINDUCTOR_STAGE_A_TESTING.md](COINDUCTOR_STAGE_A_TESTING.md).
