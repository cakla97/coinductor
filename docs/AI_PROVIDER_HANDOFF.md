...
12. ~~**Credential storage.**~~ Implemented: `coinductor/secret_store.py` stores
    credentials in the OS keychain (Windows Credential Manager via `keyring`).
    `SecretStore` reads keychain-first with a `.env` fallback, and **never writes to
    or deletes from `.env`** while a keychain is available, so an existing setup keeps
    working untouched. There is deliberately no migration button - a fresh install is
    secure by default, and an existing `.env` key moves when it is re-saved in the
    wizard or Settings. Keychain values override `.env` so a stale entry cannot shadow
    a re-entered key. If `keyring` is missing or the backend raises, saving falls back
    to the old `EnvWriter` rather than dropping keys, and the save message reports
    which store was used. Verified in a frozen PyInstaller build (backend discovery
    goes through entry points, so packaging was the real risk).
    Note: `trading_agent/cli.py` still reads `.env` only - the CLI is a
    source-checkout dev tool, so a keychain-only credential is not visible to it.
...
Repo hygiene is done: `config.toml` (gitignored) now overrides a neutralized, tracked
`config.example.toml` via `default_config_path()`, the project is MIT licensed, and
personal data was removed from the README. A sanitized diagnostics bundle
(`DiagnosticsService`, "Export diagnostics" in Settings) and the Coinductor app/exe/
installer icon (`coinductor/coinductor.ico`, generated by `packaging/make_icon.py`)
are in. Operator docs (Binance key permissions, dynamic-IP allowlists, remote/always-on
operation, AI provider presets) live in [OPERATIONS.md](OPERATIONS.md).

Credentials no longer live in plaintext: see item 12 above for `SecretStore` and the
OS keychain.

Not done yet: code signing. The installer stays unsigned by the user's decision (an
EV/OV certificate is a paid, identity-verified purchase), so SmartScreen warns on
first run. Note a portable ZIP does **not** avoid this: the extracted `.exe` is still
unsigned and Windows propagates Mark-of-the-Web to extracted files. What a release
should carry instead is `SHA256SUMS.txt` plus the from-source build instructions in
`packaging/`.
