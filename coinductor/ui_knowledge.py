...
UI_KNOWLEDGE = (
    ...
    UiKnowledgeEntry(
        ...
        "Updating is two steps: quit Coinductor from its tray icon, then install the new version over the top. No uninstall is needed, and your settings, journal, reports and API keys stay where they are. Quitting is the part that matters - a running program keeps its files open, and an install that cannot replace them leaves a mixture of old program and new data files that fails in ways looking nothing like a bad install; the tray icon's right-click menu going dead is how this was found. Since 1.5.1 you cannot get that by accident: Coinductor holds a named mutex and the installer refuses to write a single file while any copy is running, telling you which icon to close. Upgrading FROM 1.5.0 or older is the one case with no such guard, so quit it yourself first. If you do want it gone, it is a per-user install listed in Settings > Apps > Installed apps rather than the old Control Panel, and the uninstaller also sits next to the program.",
        "Aktualizace má dva kroky: ukončit Coinductor přes ikonu v oznamovací oblasti a novou verzi nainstalovat přes stávající. Odinstalovat není potřeba a nastavení, žurnál, reporty i klíče zůstanou, kde jsou. Podstatné je to ukončení - běžící program drží své soubory otevřené a instalace, která je nemůže přepsat, nechá směs starého programu a nových datových souborů; selhává to způsobem, který vůbec nevypadá jako špatná instalace, a přišlo se na to tak, že přestalo fungovat pravé tlačítko na ikoně v traye. Od verze 1.5.1 se to nemůže stát omylem: Coinductor drží pojmenovaný mutex a instalátor odmítne zapsat jediný soubor, dokud běží jakákoli kopie, a řekne vám, kterou ikonu zavřít. Jediná výjimka je upgrade Z verze 1.5.0 a starší, kde tahle pojistka ještě není - tam ji ukončete sami. Kdybyste ho přece jen chtěli odstranit, je to instalace pro jednoho uživatele, takže ji najdete v Nastavení > Aplikace > Nainstalované aplikace, ne ve starých Ovládacích panelech, a odinstalátor leží i vedle programu.",
    ),
    UiKnowledgeEntry(
        "Start with Windows",
        "Automation",
