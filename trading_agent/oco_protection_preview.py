...
class OcoProtectionPreviewBuilder:
    ...
    def build(self, live_positions: LivePositionSummary, balances: list[Balance]) -> OcoProtectionPreviewReport:
        ...
        summary = f"{ready} ready OCO protection preview(s), {blocked} blocked preview(s), {monitoring} monitored position(s). Preview only; no OCO order is submitted."
        return OcoProtectionPreviewReport(enabled=True, items=items, summary=summary)

    def _preview_position(self, position, balances: list[Balance]) -> OcoProtectionPreviewItem:
